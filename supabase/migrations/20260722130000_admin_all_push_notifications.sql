-- Admin receives every payment lifecycle notification.
-- pending  → director, admin
-- approved → employee, accounts, admin
-- denied   → employee, accounts, admin
-- paid     → director, admin
-- Actor is still excluded. Auth via latest matching lifecycle event.
...
begin
  ...
  -- Admin is on every fan-out so they always see payment activity.
  target_roles := case event_clean
    when 'pending' then array['director', 'admin']
    when 'approved' then array['employee', 'accounts', 'admin']
    when 'denied' then array['employee', 'accounts', 'admin']
